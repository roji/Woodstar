...
class Login7Message
{
    ...
    public PacketHeader Header => PacketHeader.CreateType(PacketType.TDS7Login, MessageStatus.Normal);
    ...
    public void Write<TWriter>(StreamingWriter<TWriter> writer) where TWriter : IStreamingWriter<byte>
    {
        ...
        WriteParameterHeader(ref dataOffset, _hostName);
        WriteParameterHeader(ref dataOffset, _userName);
        WriteParameterHeader(ref dataOffset, _password);
        WriteParameterHeader(ref dataOffset, _applicationName);
        WriteParameterHeader(ref dataOffset, _serverName);
        WriteParameterHeader(ref dataOffset, _extension); // TODO should be count in bytes not chars.
        WriteParameterHeader(ref dataOffset, _clientInterfaceName);
        WriteParameterHeader(ref dataOffset, _language);
        WriteParameterHeader(ref dataOffset, _database);
        writer.WriteRaw(_clientId);
        WriteParameterHeader(ref dataOffset, _sspi);
        WriteParameterHeader(ref dataOffset, _attachDbFile);
        WriteParameterHeader(ref dataOffset, _changePassword);
        writer.WriteInt(0); // cbSSPILong

        writer.WriteString(_hostName, Encoding.Unicode);
        writer.WriteString(_userName, Encoding.Unicode);
        WriteEncryptedPassword(_password);
        ...
        WriteEncryptedPassword(_changePassword ?? "");

        void WriteParameterHeader(ref ushort dataOffset, string? value)
        {
        ...
        void WriteEncryptedPassword(string value)
        {
