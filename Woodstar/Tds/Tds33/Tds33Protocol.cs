...
class Tds33Protocol : Protocol
{
    ...
    {
        readonly Tds33Protocol _instance;
...
#endif
        static async ValueTask<WriteResult> Core(Tds33Protocol instance, PgV3OperationSource source, Func<BatchWriter, TState, CancellationToken, ValueTask> batchWriter, TState state, bool flushHint = true, CancellationToken cancellationToken = default)
        {
...
//         }
//
//         static Exception CreateUnexpectedError<T>(Tds33Protocol protocol, ReadOnlySequence<byte> buffer, scoped in MessageReader<TdsPacketHeader>.ResumptionData resumptionData, long consumed, Exception? readerException = null)
//         {
...
//         }
//
//         public static T Read<T>(Tds33Protocol protocol, T message, TimeSpan timeout = default) where T : IBackendMessage<TdsPacketHeader>
//         {
//             ReadStatus status;
...
//         [AsyncMethodBuilder(typeof(PoolingAsyncValueTaskMethodBuilder<>))]
// #endif
//         public static async ValueTask<T> ReadAsync<T>(Tds33Protocol protocol, T message, CancellationToken cancellationToken = default) where T : IBackendMessage<TdsPacketHeader>
//         {
//             ReadStatus status;
//             MessageReader<TdsPacketHeader>.ResumptionData resumptionData = default;
//             long consumed = 0;
...
//         try
//         {
//             var conn = new Tds33Protocol(writer, reader, options.Encoding, pipeOptions);
...
//     }
    ...
    void CompleteOperation(PgV3OperationSource operationSource, Exception? exception)
    {
        ...
        if (exception is not null && _completingException is null)
        {
            ...
            {
                var items = (Tuple<Tds33Protocol, Exception>)state!;
    ...
    class PgV3OperationSource : OperationSource, IValueTaskSource<Operation>
    {
        ...
        public PgV3OperationSource(Tds33Protocol? protocol, bool exclusiveUse, bool pooled)
        ...
        Tds33Protocol? GetProtocol() => Unsafe.As<Protocol?, Tds33Protocol?>(ref Unsafe.AsRef(Protocol));
        ...
        public void Bind(Tds33Protocol protocol) => BindCore(protocol);
        ...
        protected override void CompleteCore(Protocol protocol, Exception? exception)
            => Unsafe.As<Protocol, Tds33Protocol>(ref protocol).CompleteOperation(this, exception);
    ...
    class PgV3OperationSource<TData> : PgV3OperationSource
    {
        ...
        public PgV3OperationSource(TData data, Tds33Protocol? protocol, bool exclusiveUse, bool pooled) : base(protocol, exclusiveUse, pooled)
        {
            _data = data;
        }
